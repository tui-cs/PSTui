...
internal sealed class OutTableViewDataSource : ITableSource
{
    ...
    ///     Rows whose original index is in <paramref name="markedIndexes" /> are always included (and appear first).
    /// </summary>
    public OutTableViewDataSource Filter(string pattern, HashSet<int> markedIndexes)
    {
        ...
        // Marked items first (always shown)
        result.AddRange(allRows.Where(r => markedIndexes.Contains(r.OriginalObjectIndex)));

        // Then unmarked items matching the filter
        result.AddRange(allRows.Where(r =>
            !markedIndexes.Contains(r.OriginalObjectIndex) &&
            RowMatchesFilter(r, pattern)));
